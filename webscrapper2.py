...
class WebScraper:
    ...
    def scrape(self):
        ...
        if scraper == "BeautifulSoup":
            ...
            if option == "Text":
                elements = browser.find_elements_by_xpath("//*[contains(text(), '{}')]".format(search))
            elif option == "Class":
                elements = browser.find_elements_by_class_name(search)
            elif option == "Tag":
                elements = browser.find_elements_by_tag_name(search)
            ...
            for element in elements:
                if option == "Text":
                    ...
                    result.append(element.get_attribute("class"))
                elif option == "Tag":
                    result.append(element.tag_name)
